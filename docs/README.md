...
1.  **Client Layer**: The primary interface for institutional stakeholders, consisting of a **Vite Portal** for human interaction and automated **CLI / Scripts** to interact with registries.
...
4.  **ZK Coprocessor**: An off-chain verification system composed of the **Erlang Daemon** (which continuously polls Starknet RPC for events and detects trust deviations) and the **Stone Prover** (a high-performance STARK prover that dynamically constructs cryptographic proofs for the detected anomalies).
5.  **Modular Package Ecosystem**: The framework is decomposed into core components spanning Cairo and Erlang:
...
6.  **Record Domains & UN SDG Metrics**: Public service data (Healthcare, Water, Industry, Education, Geology) is processed through the Triad to generate **STARK-proven metrics** mapped to United Nations Sustainable Development Goals:
