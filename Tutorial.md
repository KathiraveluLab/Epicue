...
Navigate to the **Auditor** tab to test the system's resilience manually:
- Provide the **Malicious Node Address** you suspect of Byzantine behavior.
- Input the **Deviation \%** indicating how severely the node diverged from consensus.
- Input the **Total Reviews**, the statistical sample size used to calculate the deviation.
- Input the **Off-chain STARK Proof Hash**. In a true production environment, you generate this by:
  1. Downloading the suspected node's historical public ledger data via Starknet RPC.
  2. Feeding that data into a custom anomaly detection algorithm written in the **Cairo** programming language.
  3. Executing the algorithm locally and passing the resulting trace to a local STARK prover (like the **Stone Prover**). The prover will output the cryptographic hash of your proof.
